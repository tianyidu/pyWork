...
class houseCnn:
    ...
    def addFcLayer(self,name,imgs,shape,initializer=tf.truncated_normal_initializer(stddev=1e-1)):
        with tf.variable_scope(name) as scope:
            ...
            fc = tf.add(tf.matmul(imgs,weight),bias,name=scope.name)
            if name != "softmax":
    ...
    def train(self,gloable_stept):
        ...
        pre_lables,conv1_1 = self.network()
        cross_entry = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels,logits=pre_lables,name="cross_entry")
        ...
        # 准确率计算
        correct_prediction = tf.equal(labels, tf.argmax(pre_lables, 1))
        ...
        return train_op,losses,acc,pre_lables,labels,conv1_1
...
with tf.Session() as sess:
    ...
    train_op,losses,acc,pre_lable,lables,conv1_1 = housecnn.train(global_step)
    ...
    for i in range(10000):
        ...
        _,loss,accuracy,predict,rellabel,conv = sess.run([train_op,losses,acc,pre_lable,lables,conv1_1],feed_dict={housecnn.imgs:x,housecnn.labels:y})
        if i % 10 == 0:
            print("y",y)
            ...
            print(datetime.datetime.now(),"stept",i,"loss",loss,"acc",accuracy,"predict_label",predict[0],"label",rellabel)
            print("conv",len(conv),conv.shape,conv)
